...
class ExampleReader(object):
    def __init__(self, dir_, data_name, max_len, train):
        self.dir_ = dir_
        ...
        if train:
            self.output_file = os.path.join(self.dir_, self.data_name, "train_")
        else:
            self.output_file = os.path.join(self.dir_, self.data_name, "test_")
    ...
    def get_label(self, ori_label=[], class_ids={}, class_num=40):
        ...
        labels = stack_and_pad_tensors(
            [pad_tensor(torch.tensor(input[:self.max_len]), self.max_len, padding_index=k) for input in labels]
        ).tensor.type(torch.int64).numpy()
    ...
    def get_attention_label(self, charoffset_file="", interaction_info_file="", max_len=125,):
        ...
        rf = open(os.path.join(self.dir_, self.data_name, charoffset_file), 'r', encoding='utf-8')
        while True:
            line = rf.readline()
            if line == "":
                break
        ...
        rf = open(os.path.join(self.dir_, self.data_name, interaction_info_file), 'r', encoding='utf-8')
        while True:
            line = rf.readline()
            if line == "":
                break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_label', type=int, default=40)
    ...
    e = ExampleReader(dir_, data_name, max_len=max_len, train=True)
    tri_ids_file = "tri_ids.txt"
    tri_class_id, tri_index_ids = e.read_ids(os.path.join(dir_, data_name, tri_ids_file))

    entity_ids_file = "entity_ids.txt"
    entity_class_ids, _ = e.read_ids(os.path.join(dir_, data_name, entity_ids_file))

    train_inputs, train_labels, train_entity_labels, train_deps = e.get_data()
    train_labels = e.get_label(train_labels, tri_class_id, class_num=num_label)
    ...
    e = ExampleReader(dir_, data_name, max_len=max_len, train=False)
    test_inputs, test_labels, test_entity_labels, test_deps = e.get_data()
    test_labels = e.get_label(test_labels, tri_class_id, class_num=num_label)
    test_entity_inputs = e.get_entity_input(test_entity_labels, entity_class_ids)

    wf = open(os.path.join(dir_, data_name, "tri_index_ids.pk"), 'wb')
